<template>
  <div class="hidden 2xl:flex w-[30%]" :class="[spotifyUri ? 'pt-2' : '']">
    ...
    <div v-if="!spotifyUri" class="flex justify-center items-center px-2 py-px mx-2 my-1 text-white transition-all duration-500 ease-in-out border-2 border-transparent cursor-pointer lg:py-3 lg:m-4 lg:px-4 w-fit text-ellipsis rounded-xl backdrop-blur-xl bg-opacity-40 hover:outline-2 hover:shadow-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 hover:bg-slate-600 hover:bg-opacity-30 bg-gray-300 text-sm lg:text-2xl">
      <span class="">Click on a song to preview it on Spotify</span>
    </div>
    <div class="flex items-center pb-2 pl-3 w-[64px]">
      <span v-show="loading" class="loader p-1"></span>
    </div>
  </div>
</template>
